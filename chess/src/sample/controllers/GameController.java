...
public class GameController implements Initializable {
    ...
    public static StackPane pawnStackPane = new StackPane();
    ...
    public static class Cords {
        public static Integer x1 = -1; //1 - клетка с которой ходят; 2 - на которую ходят
        public static Integer y1 = -1;
        public static Integer x2 = -1;
        public static Integer y2 = -1;
    }
    public static Cords cords;
    ...
    private void figureClicked(StackPane stackPane) {
        ...
        if(markedStackPane != null) {
            ...
            if (markedStackPane != stackPane && playerLogic.isFigure(markedStackPane)) {
                ...
                if(cellFlag != -1) {
                    setCords(markedStackPane, stackPane, cords);
    ...
    public static void setCords(StackPane markedStackPane, StackPane stackPane, Cords cords) {
        if(GridPane.getRowIndex(markedStackPane) == null) Cords.y1 = 0;
        else Cords.y1 = GridPane.getRowIndex(markedStackPane);

        if(GridPane.getColumnIndex(markedStackPane) == null) Cords.x1 = 0;
        else Cords.x1 = GridPane.getColumnIndex(markedStackPane);

        if(GridPane.getRowIndex(stackPane) == null) Cords.y2 = 0;
        else Cords.y2 = GridPane.getRowIndex(stackPane);

        if(GridPane.getColumnIndex(stackPane) == null) Cords.x2 = 0;
        else Cords.x2 = GridPane.getColumnIndex(stackPane);
    }
